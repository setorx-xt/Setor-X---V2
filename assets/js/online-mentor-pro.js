(function(){
  ...
  async function syncRanking(){
    if(!sb) return;
    try{await sb.rpc("setorx_recompute_xp_totals");}catch(e){console.warn("RPC sync unavailable",e);}
    await loadAll();toast("Ranking sincronizado.","success");
  }
  ...
  function renderMentor(){
    ...
    const opts=state.students.filter(s=>s.role!=="mentor").map(s=>`<option value="${s.id}">${esc(userName(s))} — ${esc(s.email)}</option>`).join("");
    ["#mentor-plan-student","#mentor-goal-student"].forEach(sel=>{const el=$(sel);if(el)el.innerHTML=opts||`<option value="">Nenhum aluno</option>`});
  ...
  async function savePlan(e){e.preventDefault();if(!isMentor())return toast("Apenas mentor.","error");const sid=$("#mentor-plan-student").value;if(!sid)return toast("Selecione um aluno.","error");let plan;try{plan=parsePlan($("#mentor-plan-raw").value)}catch(err){return toast(err.message,"error")}const contest=$("#mentor-plan-contest").value.trim()||plan.concurso||plan.contest||"",week=$("#mentor-plan-week").value.trim()||plan.semana||plan.week||"",msg=$("#mentor-plan-message").value.trim()||plan.mensagemMentor||plan.mentorMessage||"";plan.concurso=contest;plan.semana=week;plan.mensagemMentor=msg;const {error}=await sb.from("individual_plans").insert({user_id:sid,mentor_id:user.id,contest,week_label:week,mentor_message:msg,plan_json:plan});if(error)return toast("Erro ao enviar plano: "+error.message,"error");toast("Planejamento individual enviado.","success");e.target.reset();await loadAll()}
  ...
  function renderRanking(){const box=$("#online-ranking-board")||$("#online-ranking-list")||$("#online-ranking");if(!box)return;const html=(state.ranking||[]).map((s,i)=>`<article class="online-rank-row"><span class="rank-pos">${i+1}</span><div><strong>${esc(userName(s))}</strong><small>${esc(s.contest_target||"")} • ${n(s.xp_total)} XP</small></div></article>`).join("")||`<div class="sx-empty">Ranking vazio. Os alunos aparecem após ganharem XP.</div>`;if(box.id==="online-ranking")box.innerHTML=`<div class="section-head"><div><p class="eyebrow"><i class="fa-solid fa-trophy"></i> Ranking sincronizado</p><h2>Ranking geral</h2><span class="section-subtitle">XP sincronizado pelo Supabase.</span></div><button id="online-rank-sync" class="secondary-btn small" type="button">Sincronizar</button></div><div id="online-ranking-board" class="online-ranking-board">${html}</div>`;else box.innerHTML=html}
  function bind(){document.addEventListener("click",e=>{const tab=e.target.closest("[data-mentor-tab]");if(tab){$$('.mentor-pro-tabs button').forEach(b=>b.classList.toggle('active',b===tab));$$('.mentor-tab-panel').forEach(p=>{const active=p.dataset.mentorPanel===tab.dataset.mentorTab;p.hidden=!active;p.classList.toggle('active',active)});}const act=e.target.closest("[data-mentor-activate]");if(act)updateStudent(act.dataset.mentorActivate,"active");const pen=e.target.closest("[data-mentor-pending]");if(pen)updateStudent(pen.dataset.mentorPending,"pending");const blk=e.target.closest("[data-mentor-block]");if(blk)updateStudent(blk.dataset.mentorBlock,"blocked");const fill=e.target.closest("[data-mentor-fill-plan]");if(fill){const sel=$("#mentor-plan-student");if(sel)sel.value=fill.dataset.mentorFillPlan;document.querySelector('[data-mentor-tab="plans"]')?.click()}const cp=e.target.closest("[data-plan-copy]");if(cp){const p=state.plans.find(x=>x.id===cp.dataset.planCopy);navigator.clipboard?.writeText(JSON.stringify(p?.plan_json||{},null,2));toast("JSON do planejamento copiado.","success")}const dp=e.target.closest("[data-plan-delete]");if(dp)deletePlan(dp.dataset.planDelete);const dq=e.target.closest("[data-qx-delete]");if(dq)deleteQX(dq.dataset.qxDelete);if(e.target.closest("#mentor-refresh")||e.target.closest("#online-rank-sync"))syncRanking();if(e.target.closest("#mentor-plan-model-json")){$("#mentor-plan-raw").value=modelJSON()}if(e.target.closest("#mentor-plan-model-md")){$("#mentor-plan-raw").value=modelMD()}const b=e.target.closest("[data-mp-block]");if(b)updateProgress(b.dataset.mpBlock,false);const rb=e.target.closest("[data-mp-register]");if(rb)updateProgress(rb.dataset.mpRegister,true)},true);document.addEventListener("submit",e=>{if(e.target?.id==="mentor-plan-form")savePlan(e);if(e.target?.id==="mentor-goal-form")saveGoal(e);if(e.target?.id==="mentor-qx-form")saveQX(e)},true);window.addEventListener("setorx:auth-changed",()=>setTimeout(loadAll,180))}
