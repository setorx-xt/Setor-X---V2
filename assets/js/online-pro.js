(function(){
  ...
  function toast(msg){
    ...
    if(window.toast) window.toast(msg); else console.log('[Setor X Online]', msg);
  }
...
<section id="online-ranking" class="glass-card section-block online-pro-section online-shell-section">
  ...
  function renderRanking(){const my=rankForXP(n(profile?.xp_total)); const hero=$('#online-my-rank'),list=$('#online-ranking-list'); if(hero) hero.innerHTML=`<img src="${my.img}" alt="${esc(my.title)}"><div><p class="eyebrow">Minha patente online</p><h3>${esc(my.title)}</h3><span>${n(profile?.xp_total)} XP • ${esc(profile?.nickname||profile?.full_name||'Aluno')}</span></div>`; if(list) list.innerHTML=(state.ranking||[]).map((r,i)=>{const rk=rankForXP(n(r.xp_total));return`<div class="online-rank-row"><div class="online-rank-pos">${i+1}</div><img class="online-rank-img" src="${rk.img}" alt="${esc(rk.title)}"><div><strong>${esc(r.nickname||r.full_name||'Aluno')} — ${esc(rk.title)}</strong><span>${esc(r.contest_target||'Setor X')}</span></div><div class="online-rank-xp">${n(r.xp_total)} XP</div></div>`}).join('')||'<p class="muted">Ranking vazio.</p>'}
