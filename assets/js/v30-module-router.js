...
(function(){
  ...
  function openVadeLawBridge(){
    apply("vade", true);
    setTimeout(()=>{
      if(window.SetorXVadeMecum?.openSelected) window.SetorXVadeMecum.openSelected();
      else if(window.SetorXVadeMecum?.open) window.SetorXVadeMecum.open(window.SetorXVadeMecum.activeLaw?.());
      else document.getElementById("vade-mecum")?.scrollIntoView({behavior:"smooth",block:"start"});
    },160);
  }
  function ensureVadeBridgeButtons(){
    const vade=document.getElementById("vade-mecum");
    const lei=document.getElementById("lei-seca-pro");
    if(vade && !document.getElementById("sx-open-lei-seca-from-vade")){
      const head=vade.querySelector(".section-head")||vade;
      head.insertAdjacentHTML("beforeend",`<div class="sx-vade-bridge"><button id="sx-open-lei-seca-from-vade" class="secondary-btn small" type="button"><i class="fa-solid fa-highlighter"></i> Abrir lei para grifar</button></div>`);
    }
    if(lei && !document.getElementById("sx-open-vade-from-lei")){
      const head=lei.querySelector(".section-head")||lei;
      head.insertAdjacentHTML("beforeend",`<div class="sx-vade-bridge"><button id="sx-open-vade-from-lei" class="ghost-btn small" type="button"><i class="fa-solid fa-highlighter"></i> Abrir lei no Vade Mecum</button></div>`);
    }
  }
  ...
  function bind(){
    ...
    document.addEventListener("click",e=>{
      if(e.target.closest("#sx-open-lei-seca-from-vade")){e.preventDefault();openVadeLawBridge();}
      if(e.target.closest("#sx-open-vade-from-lei")){e.preventDefault();openVadeLawBridge();}
      if(e.target.closest("#lsx-open-vade")){e.preventDefault();openVadeLawBridge();}
    },true);
